...
export default function DataTableDescription({ 
  ...
  const [sortColumn, setSortColumn] = useState<string | null>(columns.length > 0 ? columns[0].key : null);
  ...
  return (
    <div className={`chart-description ${backgroundColor} overflow-hidden`}>
      ...
      <button
        ...
        tabIndex={0}
        className={`w-full px-0 py-2 flex items-start justify-start text-left focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-inset transition-colors duration-200 ${isExpanded ? '' : 'border-b border-[#78787878]'}`}
      >
                      ...
                      <tr className="border-b border-gray-200">
                        {columns.map((column) => {
                          ...
                          const sortAriaLabel = isSorted 
                            ? `Sort by ${column.label}, ${sortDirection === 'asc' ? 'ascending' : 'descending'}. Click or press Enter to reverse.`
